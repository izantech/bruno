...
const createCapacitorBackend = () => {
  ...
  const plugin = window.Capacitor.Plugins.BrunoFilesystem;
  ...
  const synthesizeTreeForCollection = async (token, uid, format) => {
    ...
    const { entries } = await plugin.readDirTree({ path: token });
  ...
  const openWorkspace = async () => {
    ...
    const { collections } = await plugin.scanWorkspace();
  ...
  const invoke = async (channel, ...args) => {
    ...
    switch (channel) {
      ...
      case 'renderer:mount-collection': {
        ...
        return plugin.mountCollection({ token: collectionPathname }).then(({ tempDirectoryToken }) => {
      ...
      case 'renderer:save-request': {
        ...
        return plugin.writeFile({ path: pathname, content: stringifyRequest(request, { format }) }).then(() => {
      ...
      case 'renderer:save-multiple-requests': {
        ...
        return Promise.all(
          requestsToSave.map(({ pathname, item, format }) =>
            plugin.writeFile({ path: pathname, content: stringifyRequest(item, { format }) }))
        ).then(() => undefined);
      }
      ...
      case 'renderer:save-scratch-request': {
        ...
        return plugin.writeFile({ path: newPathname, content: stringifyRequest(request, { format }) }).then(() => ({ newPathname }));
      }

      case 'renderer:delete-item': {
        const [pathname, type] = args;
        const recursive = type === 'folder';
        return plugin.deleteEntry({ path: pathname, recursive }).then(() => {
      ...
      case 'renderer:delete-transient-requests': {
        ...
        return Promise.all(filePaths.map((path) => plugin.deleteEntry({ path }))).then(() => ({ deleted: filePaths, skipped: [], errors: [] }));
      }

      case 'renderer:new-folder': {
        const [{ pathname, folderData, format }] = args;
        return plugin.mkdir({ path: pathname }).then(() => {
          if (!folderData) return undefined;
          const folderFile = joinPath(pathname, format === 'yml' ? 'folder.yml' : 'folder.bru');
          return plugin.writeFile({ path: folderFile, content: stringifyFolder(folderData, { format }) });
        }).then(() => {
      ...
      case 'renderer:save-folder-root': {
        ...
        return plugin.writeFile({ path: folderFile, content: stringifyFolder(folder.root, { format }) }).then(() => undefined);
      }
      ...
      case 'renderer:rename-item-filename': {
        ...
        return plugin.rename({ from: oldPath, to: newPath }).then(({ to }) => to);
      }

      case 'renderer:move-file-item':
      case 'renderer:move-folder-item': {
        const [itemPath, destinationPath] = args;
        return plugin.rename({ from: itemPath, to: destinationPath }).then(() => undefined);
      }

      case 'renderer:move-item': {
        const [{ targetDirname, sourcePathname }] = args;
        return plugin.rename({ from: sourcePathname, to: joinPath(targetDirname, basename(sourcePathname)) }).then(() => undefined);
      }

      case 'renderer:move-item-cross-format': {
        const [{ targetDirname, sourcePathname }] = args;
        const newPathname = joinPath(targetDirname, basename(sourcePathname));
        return plugin.rename({ from: sourcePathname, to: newPathname }).then(({ to }) => ({ newPathname: to }));
      }

      case 'renderer:clone-folder': {
        const [itemFolder, collectionPath] = args;
        return plugin.copyTree({ from: itemFolder, to: collectionPath }).then(() => undefined);
      }
      ...
      case 'renderer:save-collection-root': {
        ...
        return plugin.writeFile({ path: rootFile, content: stringifyCollection(collectionRoot, brunoConfig, { format }) }).then(() => undefined);
      }
      ...
      case 'renderer:update-bruno-config': {
        ...
        return plugin.writeFile({ path: configFile, content }).then(() => undefined);
      }

      case 'renderer:get-collection-json': {
        const [collectionPath] = args;
        return plugin.readDirTree({ path: collectionPath }).then(({ entries }) => ({ entries }));
      }
      ...
      case 'renderer:save-environment': {
        ...
        return plugin.writeFile({ path: envFile, content: stringifyEnvironment(env, { format: envFormat }) }).then(() => undefined);
      }
      ...
      case 'renderer:rename-environment': {
        ...
        return plugin.rename({ from, to }).then(() => undefined);
      }

      case 'renderer:delete-environment': {
        const [collectionPathname, environmentName] = args;
        return plugin.deleteEntry({ path: joinPath(collectionPathname, 'environments', `${environmentName}.bru`) }).then(() => undefined);
      }
      ...
      case 'renderer:save-dotenv-raw': {
        ...
        return plugin.writeFile({ path: joinPath(collectionPathname, filename || '.env'), content }).then(() => ({ success: true }));
      }

      case 'renderer:create-dotenv-file': {
        const [collectionPathname, filename] = args;
        const name = filename || '.env';
        return plugin.writeFile({ path: joinPath(collectionPathname, name), content: '' }).then(() => ({ success: true, filename: name }));
      }

      case 'renderer:delete-dotenv-file': {
        const [collectionPathname, filename] = args;
        return plugin.deleteEntry({ path: joinPath(collectionPathname, filename) }).then(() => ({ success: true }));
      }

      case 'renderer:exists-sync': {
        const [filePath] = args;
        return plugin.exists({ path: filePath }).then(({ exists }) => exists);
      }

      case 'renderer:is-directory': {
        const [pathname] = args;
        return plugin.stat({ path: pathname }).then(({ isDirectory }) => isDirectory);
      }
      ...
      case 'renderer:find-unique-folder-name': {
        ...
        return plugin.readDirTree({ path: location }).then(({ entries }) => {
      ...
      case 'renderer:snapshot:get-tabs':
        return plugin.snapshotGet().then(({ snapshot }) => snapshot);

      case 'renderer:snapshot:save':
      case 'renderer:update-ui-state-snapshot': {
        const [snapshot] = args;
        return plugin.snapshotSave({ snapshot }).then(({ ok }) => ok);
      }
  ...
  const on = (channel, handler) => {
    ...
    plugin.addListener(channel, adapt).then((registration) => {
